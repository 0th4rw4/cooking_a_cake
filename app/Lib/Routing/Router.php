 ...
 * @var string
 */
	const ACTION = 'index|show|add|create|edit|update|remove|del|delete|view|item|find';
 ...
 */
	protected static $_resourceMap = array(
		array('action' => 'find',	'method' => 'GET',		'id' => false),
 ...
 */
	public static function mapResources($urlName, $options = array()) {
		...
		$action_prefix = ($options['action_prefix'])?$options['action_prefix'].'_':'';

		foreach ((array)$controller as $name) {
			$url = Inflector::underscore($urlName);
			foreach (self::$_resourceMap as $params) {
				...
				Router::connect($url,
					array(
						...
						'action' => $action_prefix.$params['action'],
